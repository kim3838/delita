...
class LeaveService
{
    public function balanceByPeriod(EmployeeLeaveType $employeeLeaveType, $upToDate): void
    {
        ...
        $employmentProfileSeries = "
            WITH RECURSIVE employment_profile_series(date_series) AS (
                SELECT employment_profile_series_sub.employment_profile_start
                FROM (
                    SELECT
        ...
        $queryBuilder = app(QueryBuilder::class)
            ->fromSub($employmentProfileByDateSeriesQueryBuilder, 'employment_profile_by_date_series_sub')
            ->select([
